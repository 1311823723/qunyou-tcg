...
| 最后更新 | 2026-09-04 |
| 当前进度 | 10 张自动预组本体模块化；120 张角色全部注册；正式预组已覆盖全部120张角色，南山五的破魔组与花生壳的交叉组暂未开放自动模式 |
...
1. 10 套自动预组引用的角色均已完成，自动房可选择10套已注册本体预组；破魔组与交叉组保留为经典预组。
2. 预组迁移已完成，共享角色只实现一次；后续持续进行跨预组、自选组合的回归和实战验证。
3. 全部120张角色均已注册并被正式预组覆盖。自选卡组开放120张角色和10个已解锁本体；南山五与花生壳尚未实现自动本体。
...
- `characters/extra.mts` 保留最初19张补充角色的集中注册；连携组与巡界组已复用其中16张，当前仅3张仍未进入预组。公共引擎处理真实标记牌、分次闪避、牌名封锁与伤害续接，不在前端写角色效果。
...
## 交互描述与提交前草稿（2026-09-05）

`src/lib/auto-action-types.ts` 为 Worker 与前端共享 `AutoLegalAction` 类型。在既有 `type`、`payload`、`selection` 上增加可选 `interaction`：

- `label`：动作名称。
- `quickPlay`：仅普通手牌可为 true；是输入体验描述，不是服务器授权凭据。
- `target: { playerId, slotIndex? }`：公开目标语义，不包含对手暗置角色 ID。
- `cost: { kind, amount?, fixedIds?, options? }`：当前费用及分支，kind 为 rest / retire / none / choice。引用的费用卡均属于动作发起者已知区域。

快照增加 `game.unavailableReasons`，按己方卡例 ID 提供不可用原因。观战者得到空对象，对手快照不包含其他玩家的私有卡例。合法行为仍通过现有执行入口、费用与目标校验授权。

费用预览是只读计算，不消耗回合修正。`previewSkillCost` 按实际支付顺序应用费用修正；同等费用读取当前技能触发事件。`skillCostSelection` 使用这一结果提供选择数量，实际支付仍由 `paySkillCost` 完成。隐藏对手技能的后续触发不能提前出现在费用描述中。

客户端 `AutoInteraction` 集中管理可返回的本地草稿与已发送请求。发送后清除本地返回历史；重连恢复服务器提示，不重发不确定请求。牌序草稿按 promptId 保存，提交前验证完整且不重复，再转换成现有 `choice:submit` 顺序字符串，因此无需变更持久化格式。

自动交互回归见 `e2e/auto-interactions.spec.ts`；描述、费用预览及隐私回归见 `tools/battle/auto-interaction-description.test.mjs`。
