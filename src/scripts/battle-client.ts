...
import {
  actionFeedback as buildActionFeedback,
  actionLockKey,
  actionTargetKey as buildActionTargetKey,
...
function actionFeedback(type: string, payload: Record<string, unknown>) {
  return buildActionFeedback(type, payload, cardDisplayName(String(payload.instanceId || "")));
}
...
function actionTargetKey(type: string, payload: Record<string, unknown>) {
  ...
  const markerOwnerId = typeof payload.markerId === "string"
    ...
    : undefined;
  return buildActionTargetKey(type, payload, { you, markerOwnerId });
}
...
function showCustomDeckEditor(me: PlayerView) {
  mountCustomDeckEditor({
    dialog, dialogContent, catalog, bodyCatalogCards, characterCatalogCards,
    deck: readCustomDeck(me), onPreview: showCustomCardPreview, openBattleDialog,
    onSave: (customDeck) => {
